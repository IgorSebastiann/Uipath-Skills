...
## Instalação interativa para agentes de IA

Esta é a forma recomendada para obter a experiência visual do `skills` CLI, com seleção de escopo, método de instalação e agentes compatíveis:

```bash
npx skills add IgorSebastiann/Uipath-Skills
```

O instalador detecta os agentes disponíveis na máquina. Se nenhum for detectado, ele permite escolher manualmente entre Codex, Claude Code, Cursor, GitHub Copilot, Windsurf, Gemini CLI, Cline e dezenas de outros.

Para instalar globalmente em agentes específicos sem perguntas:
...
```bash
# Ver a skill disponível sem instalar
npx skills add IgorSebastiann/Uipath-Skills --list

# Usar temporariamente sem instalar
...
O CLI requer Node.js e pode ser executado diretamente com `npx`, sem instalação global.
